...
def mixing_time(P):
    ...
    Mixing time ≈ 1 / (1 - λ₂) where λ₂ is the second-largest eigenvalue.
    """
    try:
        ...
        # Handle numerical errors and edge cases
        lambda_2 = eigvals[1] if len(eigvals) > 1 else 0
        
        # Ensure lambda_2 is less than 1 (for stochastic matrices)
        lambda_2 = min(lambda_2, 0.9999)
        lambda_2 = max(lambda_2, 0)
        
        gap = 1 - lambda_2
        
        if gap < 1e-6:  # Very small gap means very slow mixing
            return 10000.0
        ...
        return 1000.0  # Return a default value if computation fails
...
def build_random_graph(n, p=0.3):
    """
    Build a random connected graph with edge probability p.
    Ensures all nodes have at least one outgoing edge.
    """
    adj = (np.random.rand(n, n) < p).astype(int)
    np.fill_diagonal(adj, 0)
    
    # Ensure each node has at least one outgoing edge
    # This prevents isolated nodes or dead ends
    for i in range(n):
        if adj[i].sum() == 0:
            # Add at least one random edge
    ...
    P[np.isnan(P)] = 1.0 / n  # Fallback in case of division issues
